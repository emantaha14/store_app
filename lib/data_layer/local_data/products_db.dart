...
class ProductsDb{
  ...
  initialDb()async{
    ...
    String path = join(databasePath, 'store.dp');
    Database recipeDb = await openDatabase(path, onCreate: _onCreate,version: 1);
    return recipeDb;
  }

  _onCreate(Database dp, int version){
    dp.execute('CREATE TABLE StoreApp (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, category TEXT NOT NULL, price TEXT NOT NULL, image TEXT NOT NULL, fakeId INT NOT NULL UNIQUE)');
    print('onCreate =======================');
  }
  ...
  onDeleteDataBase() async {
    String databasePath = await getDatabasesPath();
    String pathName = join(databasePath, 'store.dp');
